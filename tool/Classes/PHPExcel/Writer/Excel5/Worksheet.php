...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter
{
     ...
     */
    public function __construct( &$str_total, &$str_unique, &$str_table, &$colors, $parser, $preCalculateFormulas,
        $phpSheet )
    {
    ...
    function close()
    {
        ...
        $num_sheets = $_phpSheet->getParent()
                                ->getSheetCount();
        ...
        // Column dimensions
        if ( ( $defaultWidth = $_phpSheet->getDefaultColumnDimension()
                                         ->getWidth() ) < 0
        ) {
        ...
        $autoFilterRange = $_phpSheet->getAutoFilter()
                                     ->getRange();
            ...
            if ( $cVal instanceof PHPExcel_RichText ) {
                ...
                foreach ( $elements as $element ) {
                    ...
                    if ( $element instanceof PHPExcel_RichText_Run ) {
                        $str_fontidx = $this->_fntHashIndex[ $element->getFont()
                                                                     ->getHashCode() ];
                    } else {
                        $str_fontidx = 0;
                    }
    ...
    function _writeUrlExternal( $row1, $col1, $row2, $col2, $url )
    {
        ...
        $data = pack( "vvvv", $row1, $row2, $col1, $col2 ) . $unknown1 . $link_type . $unknown2 . $up_count .
                $dir_short_len . $dir_short . $unknown3 . $stream_len; /* .
          $dir_long_len .
          $unknown4	 .
          $dir_long	 .
          $sheet_len	.
    ...
    private function _writeDimensions()
    {
        ...
        $length = 0x000E;
        $data = pack( 'VVvvv', $this->_firstRowIndex, $this->_lastRowIndex + 1, $this->_firstColumnIndex,
                      $this->_lastColumnIndex + 1, 0x0000 // reserved
        );
    ...
    private function _writeWindow2()
    {
        ...
        $fSelected = ( $this->_phpSheet === $this->_phpSheet->getParent()
                                                            ->getActiveSheet() ) ? 1 : 0;
        $fPaged = 1;      // 2
        $fPageBreakPreview = $this->_phpSheet->getSheetView()
        ...
        $zoom_factor_page_break = ( $fPageBreakPreview ? $this->_phpSheet->getSheetView()
                                                                         ->getZoomScale() : 0x0000 );
        $zoom_factor_normal = $this->_phpSheet->getSheetView()
                                              ->getZoomScaleNormal();
    ...
    private function _writeSheetLayout()
    {
        ...
        $recordData = pack( 'vvVVVvv', 0x0862, 0x0000  // unused
            ...
            , $this->_colors[ $this->_phpSheet->getTabColor()
                                              ->getRGB() ] // color index
            , 0x0000  // unused
        );
    ...
    private function _writeSheetProtection()
    {
        ...
        // prepare options
        $options = ( int )!$this->_phpSheet->getProtection()
                                           ->getObjects() | ( int )!$this->_phpSheet->getProtection()
                                                                                    ->getScenarios() << 1 |
                   ( int )!$this->_phpSheet->getProtection()
                   ...
                   ( int )!$this->_phpSheet->getProtection()
                                           ->getInsertRows() << 6 | ( int )!$this->_phpSheet->getProtection()
                                                                                            ->getInsertHyperlinks() <<
                                                                    7 | ( int )!$this->_phpSheet->getProtection()
                                                                                                ->getDeleteColumns() <<
                                                                        8 | ( int )!$this->_phpSheet->getProtection()
                                                                                                    ->getDeleteRows() <<
                                                                            9 |
                   ( int )!$this->_phpSheet->getProtection()
    ...
    private function _writeSetup()
    {
        ...
        $iPaperSize = $this->_phpSheet->getPageSetup()
                                      ->getPaperSize(); // Paper size
        
        $iScale = $this->_phpSheet->getPageSetup()
                                  ->getScale() ? $this->_phpSheet->getPageSetup()
                                                                 ->getScale() : 100;   // Print scaling factor
        
        $iPageStart = 0x01;     // Starting page number
        $iFitWidth = ( int )$this->_phpSheet->getPageSetup()
                                            ->getFitToWidth(); // Fit to number of pages wide
        $iFitHeight = ( int )$this->_phpSheet->getPageSetup()
                                             ->getFitToHeight(); // Fit to number of pages high
        $grbit = 0x00;     // Option flags
        $iRes = 0x0258;      // Print resolution
        $iVRes = 0x0258;      // Vertical print resolution
        
        $numHdr = $this->_phpSheet->getPageMargins()
                                  ->getHeader();  // Header Margin
        
        $numFtr = $this->_phpSheet->getPageMargins()
                                  ->getFooter();   // Footer Margin
        $iCopies = 0x01;     // Number of copies
        
        $fLeftToRight = 0x0;      // Print over then down
        // Page orientation
        $fLandscape = ( $this->_phpSheet->getPageSetup()
    ...
    private function _writeHcenter()
    {
        ...
        $fHCenter = $this->_phpSheet->getPageSetup()
    ...
    private function _writeVcenter()
    {
        ...
        $fVCenter = $this->_phpSheet->getPageSetup()
    ...
    private function _writeMarginLeft()
    {
        ...
        $margin = $this->_phpSheet->getPageMargins()
    ...
    private function _writeWsbool()
    {
        ...
        if ( $this->_phpSheet->getPageSetup()
                             ->getFitToPage()
        ) {
    ...
    private function _writeProtect()
    {
        ...
        if ( !$this->_phpSheet->getProtection()
                              ->getSheet()
        ) {
            return;
        }
    ...
    private function _writePassword()
    {
        ...
        if ( !$this->_phpSheet->getProtection()
                              ->getSheet() || !$this->_phpSheet->getProtection()
                                                               ->getPassword()
        ) {
            return;
        }
        ...
        $wPassword = hexdec( $this->_phpSheet->getProtection()
    ...
    {
        $bitmap_array =
            ( is_resource( $bitmap ) ? $this->_processBitmapGd( $bitmap ) : $this->_processBitmap( $bitmap ) );
    ...
    function _positionImage( $col_start, $row_start, $x1, $y1, $width, $height )
    {
        ...
        if ( $x1 >=
             PHPExcel_Shared_Excel5::sizeCol( $this->_phpSheet, PHPExcel_Cell::stringFromColumnIndex( $col_start ) )
        ) {
            $x1 = 0;
        }
        ...
        while ( $width >= PHPExcel_Shared_Excel5::sizeCol( $this->_phpSheet,
            ...
            $width -= PHPExcel_Shared_Excel5::sizeCol( $this->_phpSheet,
                                                       PHPExcel_Cell::stringFromColumnIndex( $col_end ) );
            ++$col_end;
        }
        ...
        //
        if ( PHPExcel_Shared_Excel5::sizeCol( $this->_phpSheet, PHPExcel_Cell::stringFromColumnIndex( $col_start ) ) ==
             0
        ) {
            return;
        }
        if ( PHPExcel_Shared_Excel5::sizeCol( $this->_phpSheet, PHPExcel_Cell::stringFromColumnIndex( $col_end ) ) ==
             0
        ) {
            return;
        }
        ...
        $x2 = $width /
              PHPExcel_Shared_Excel5::sizeCol( $this->_phpSheet, PHPExcel_Cell::stringFromColumnIndex( $col_end ) ) *
              1024; // Distance to right side of object
        $y2 = $height / PHPExcel_Shared_Excel5::sizeRow( $this->_phpSheet, $row_end + 1 ) *
    ...
    private function _writeZoom()
    {
        ...
        $header = pack( "vv", $record, $length );
        $data = pack( "vv", $this->_phpSheet->getSheetView()
                                            ->getZoomScale(), 100 );
        $this->_append( $header . $data );
    }
    ...
    private function _writePageLayoutView()
    {
        ...
        $wScalvePLV = $this->_phpSheet->getSheetView()
                                      ->getZoomScale(); // 2
        // The options flags that comprise $grbit
        if ( $this->_phpSheet->getSheetView()
    ...
    private function _writeCFRule( PHPExcel_Style_Conditional $conditional )
    {
        ...
        if ( $bAlignHz == 0 || $bAlignVt == 0 || $bAlignWrapTx == 0 || $bTxRotation == 0 || $bIndent == 0 ||
             $bShrinkToFit == 0
        ) {
            $bFormatAlign = 1;
        } else {
        ...
        if ( $bFormatFont == 1 ) {
            ...
            if ( $conditional->getStyle()
                ...
                $dataBlockFont =
                    pack( 'VVVVVVVV', 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
