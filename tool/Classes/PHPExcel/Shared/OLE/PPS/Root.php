...
class PHPExcel_Shared_OLE_PPS_Root extends PHPExcel_Shared_OLE_PPS
{
    ...
    public function save( $filename )
    {
        ...
        $this->_BIG_BLOCK_SIZE =
            pow( 2, ( ( isset( $this->_BIG_BLOCK_SIZE ) ) ? self::_adjust2( $this->_BIG_BLOCK_SIZE ) : 9 ) );
        $this->_SMALL_BLOCK_SIZE =
            pow( 2, ( ( isset( $this->_SMALL_BLOCK_SIZE ) ) ? self::_adjust2( $this->_SMALL_BLOCK_SIZE ) : 6 ) );
    ...
    public function _saveHeader( $iSBDcnt, $iBBcnt, $iPPScnt )
    {
        ...
        fwrite( $FILE,
                ...
                "\x00\x00\x00\x00" . pack( "v", 0x3b ) . pack( "v", 0x03 ) . pack( "v", -2 ) . pack( "v", 9 ) .
                pack( "v", 6 ) . pack( "v", 0 ) . "\x00\x00\x00\x00" . "\x00\x00\x00\x00" . pack( "V", $iBdCnt ) .
    ...
    public function _saveBigData( $iStBlk, &$raList )
    {
        ...
        for ( $i = 0; $i < $iCount; ++$i ) {
            if ( $raList[ $i ]->Type != PHPExcel_Shared_OLE::OLE_PPS_TYPE_DIR ) {
                $raList[ $i ]->Size = $raList[ $i ]->_DataLen();
                if ( ( $raList[ $i ]->Size >= PHPExcel_Shared_OLE::OLE_DATA_SIZE_SMALL ) ||
                     ( ( $raList[ $i ]->Type == PHPExcel_Shared_OLE::OLE_PPS_TYPE_ROOT ) &&
                    ...
                    if ( $raList[ $i ]->Size % $this->_BIG_BLOCK_SIZE ) {
                        fwrite( $FILE, str_repeat( "\x00", $this->_BIG_BLOCK_SIZE -
            ...
            if ( $raList[ $i ]->Type == PHPExcel_Shared_OLE::OLE_PPS_TYPE_FILE ) {
                ...
                if ( $raList[ $i ]->Size < PHPExcel_Shared_OLE::OLE_DATA_SIZE_SMALL ) {
                    ...
                    if ( $raList[ $i ]->Size % $this->_SMALL_BLOCK_SIZE ) {
                        $sRes .= str_repeat( "\x00", $this->_SMALL_BLOCK_SIZE -
