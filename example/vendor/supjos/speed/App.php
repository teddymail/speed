...
use supjos\base\RequestInterface;
use supjos\exception\ClassImplementException;
...
class App
{
     ...
     * @var type
     */
    public static $aliases = ['@supjos' => __DIR__];
    /**
     ...
     * @param Closure|string $callBackFunc The callback function or the implement class [[ RequestController ]] or
     *                                     class implement from the [[ RequestInterface ]] interface to do the job
     */
     ...
     * @param $callBackFunc  |string  The request callback function or the class implement the interface [[
     *                       RequestInterface ]] or derive class from [[ RequestController ]], to deal with the logic
     *
     * @throws \supjos\exception\ClassImplementException
     */
        ...
        if ( $request->{'is' . ucfirst( strtolower( $method ) )}() ) {
            ...
            if ( !empty( $matches ) || ( $requestUrl == '/' && empty( $request->getPathInfo() ) ) ) {
                ...
                if ( is_callable( $callBackFunc ) && $callBackFunc instanceof Closure ) {
                    ...
                    call_user_func_array( $callBackFunc, [$request, $response] );
                    
                } elseif ( App::createObject( $callBackFunc ) instanceof RequestInterface ) {
                    
                    if ( method_exists( App::createObject( $callBackFunc ), 'init' ) ) {
                        
                        $pathArr = explode( '/', trim( $request->getPathInfo(), '/' ) );
                        $request->paramArg = $pathArr[ count( $pathArr ) - 1 ];
                        App::createObject( $callBackFunc )
                           ->init();
                        
                    } else {
                        throw new ClassImplementException( "Abstract method init not implements." );
                    }
                }
            } else {
