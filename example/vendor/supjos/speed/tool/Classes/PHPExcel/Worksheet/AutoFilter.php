...
class PHPExcel_Worksheet_AutoFilter
{
    ...
    public function showHideRows()
    {
        ...
        $this->_workSheet->getRowDimension( $rangeStart[ 1 ] )
                         ->setVisible( TRUE );
            ...
            switch ( $filterColumn->getFilterType() ) {
                case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_FILTER :
                    ...
                    if ( $ruleType == PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_FILTER ) {
                        ...
                        $columnFilterTests[ $columnID ] = ['method'    => '_filterTestInSimpleDataSet',
                        ...
                        $columnFilterTests[ $columnID ] = ['method'    => '_filterTestInDateGroupSet',
                ...
                case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_CUSTOMFILTER :
                    ...
                    $columnFilterTests[ $columnID ] = ['method'    => '_filterTestInCustomDataSet',
                    ...
                    foreach ( $rules as $rule ) {
                        ...
                        if ( ( $dynamicRuleType ==
                            ...
                            $average = PHPExcel_Calculation::getInstance()
                                                           ->calculateFormula( $averageFormula, NULL,
                            ...
                            if ( $dynamicRuleType{0} == 'M' || $dynamicRuleType{0} == 'Q' ) {
                                ...
                                $columnFilterTests[ $columnID ] =
                                    ['method' => '_filterTestInPeriodDateSet', 'arguments' => $ruleValues];
                                $filterColumn->setAttributes( [] );
                            } else {
                                //	Date Range
                                $columnFilterTests[ $columnID ] =
                                    $this->_dynamicFilterDateRange( $dynamicRuleType, $filterColumn );
                                break;
                            }
                        }
                    }
                ...
                case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_TOPTENFILTER :
                    ...
                    $maxVal =
                        $this->_calculateTopTenValue( $columnID, $rangeStart[ 1 ] + 1, $rangeEnd[ 1 ], $toptenRuleType,
                                                      $ruleValue );
        ...
        for ( $row = $rangeStart[ 1 ] + 1; $row <= $rangeEnd[ 1 ]; ++$row ) {
            ...
            foreach ( $columnFilterTests as $columnID => $columnFilterTest ) {
                ...
                $cellValue = $this->_workSheet->getCell( $columnID . $row )
                                              ->getCalculatedValue();
                //				echo 'Value is ',$cellValue,PHP_EOL;
                //	Execute the filter test
                $result = $result &&
                          call_user_func_array( ['PHPExcel_Worksheet_AutoFilter', $columnFilterTest[ 'method' ]],
            ...
            $this->_workSheet->getRowDimension( $row )
                             ->setVisible( $result );
        }
    ...
    private function _dynamicFilterDateRange( $dynamicRuleType, &$filterColumn )
    {
        ...
        switch ( $dynamicRuleType ) {
            ...
            case PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_LASTMONTH :
                $baseDate =
                    strtotime( '-1 month', gmmktime( 0, 0, 0, 1, date( 'm', $baseDate ), date( 'Y', $baseDate ) ) );
                break;
            ...
            case PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_LASTQUARTER :
                $baseDate =
                    strtotime( '-3 month', gmmktime( 0, 0, 0, 1, date( 'm', $baseDate ), date( 'Y', $baseDate ) ) );
                break;
            ...
            case PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_LASTYEAR :
                $baseDate =
                    strtotime( '-1 year', gmmktime( 0, 0, 0, 1, date( 'm', $baseDate ), date( 'Y', $baseDate ) ) );
                break;
        ...
        switch ( $dynamicRuleType ) {
            ...
            case PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTYEAR :
                $maxVal =
                    ( int )PHPExcel_Shared_Date::PHPToExcel( gmmktime( 0, 0, 0, 31, 12, date( 'Y', $baseDate ) ) );
                ++$maxVal;
                $val = ( int )PHPExcel_Shared_Date::PHPToExcel( gmmktime( 0, 0, 0, 1, 1, date( 'Y', $baseDate ) ) );
                break;
        ...
        return ['method'    => '_filterTestInCustomDataSet', 'arguments' => ['filterRules' => $ruleValues,
    ...
    private function _calculateTopTenValue( $columnID, $startRow, $endRow, $ruleType, $ruleValue )
    {
        ...
        $dataValues = PHPExcel_Calculation_Functions::flattenArray( $this->_workSheet->rangeToArray( $range, NULL, TRUE,
                                                                                                     FALSE ) );
