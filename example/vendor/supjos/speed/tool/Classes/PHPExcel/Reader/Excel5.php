...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
        ...
        foreach ( $this->_sheets as $sheet ) {
            ...
            foreach ( $this->_objs as $n => $obj ) {
                ...
                if ( isset( $allSpContainers[ $n + 1 ] ) && is_object( $allSpContainers[ $n + 1 ] ) ) {
                    ...
                    list( $startColumn, $startRow ) =
                        PHPExcel_Cell::coordinateFromString( $spContainer->getStartCoordinates() );
                    list( $endColumn, $endRow ) =
                        PHPExcel_Cell::coordinateFromString( $spContainer->getEndCoordinates() );
                    ...
                    $width =
                        PHPExcel_Shared_Excel5::getDistanceX( $this->_phpSheet, $startColumn, $startOffsetX, $endColumn,
                                                              $endOffsetX );
            ...
            if ( $definedName[ 'isBuiltInName' ] ) {
                            ...
                            if ( count( $explodes ) == 2 ) {
                                if ( $docSheet = $this->_phpExcel->getSheetByName( $explodes[ 0 ] ) ) {
                                    ...
                                    if ( count( $coordinateStrings ) == 2 ) {
                                        list( $firstColumn, $firstRow ) =
                                            PHPExcel_Cell::coordinateFromString( $coordinateStrings[ 0 ] );
                                        list( $lastColumn, $lastRow ) =
                                            PHPExcel_Cell::coordinateFromString( $coordinateStrings[ 1 ] );
                                        
                                        if ( $firstColumn == 'A' and $lastColumn == 'IV' ) {
                                            // then we have repeating rows
                                            $docSheet->getPageSetup()
                                                     ->setRowsToRepeatAtTop( [$firstRow, $lastRow] );
                                        } elseif ( $firstRow == 1 and $lastRow == 65536 ) {
                                            // then we have repeating columns
                                            $docSheet->getPageSetup()
                                                     ->setColumnsToRepeatAtLeft( [$firstColumn, $lastColumn] );
                                        }
                                    }
                                }
                            }
                ...
                if ( count( $explodes ) == 2 ) {
                    if ( ( $docSheet = $this->_phpExcel->getSheetByName( $explodes[ 0 ] ) ) ||
                        ...
                        $scope = ( $definedName[ 'scope' ] == 0 ) ? NULL :
                            $this->_phpExcel->getSheetByName( $this->_sheets[ $definedName[ 'scope' ] - 1 ][ 'name' ] );
                        
                        $this->_phpExcel->addNamedRange( new PHPExcel_NamedRange( ( string )$definedName[ 'name' ],
                                                                                  $docSheet, $extractedRange,
                                                                                  $localOnly, $scope ) );
                    }
                } else {
                    //	Named Value
    ...
    private function _readFilepass()
    {
        ...
        if ( !$this->_verifyPassword( 'VelvetSweatshop', substr( $recordData, 6, 16 ), substr( $recordData, 22, 16 ),
                                      substr( $recordData, 38, 16 ), $this->_md5Ctxt )
        ) {
    ...
    private function _readByteStringShort( $subData )
    {
        ...
        return ['value' => $value, 'size' => 1 + $ln,// size in bytes of data structure
        ];
    }
    ...
    private function _readXf()
    {
        ...
        if ( !$this->_readDataOnly ) {
            ...
            $objStyle->getProtection()
                     ->setLocked( $isLocked ? PHPExcel_Style_Protection::PROTECTION_INHERIT :
                                      PHPExcel_Style_Protection::PROTECTION_UNPROTECTED );
            
            // bit 1; mask 0x02; 1 = Formula is hidden
            $isHidden = ( 0x02 & $xfTypeProt ) >> 1;
            $objStyle->getProtection()
                     ->setHidden( $isHidden ? PHPExcel_Style_Protection::PROTECTION_PROTECTED :
                                      PHPExcel_Style_Protection::PROTECTION_UNPROTECTED );
            ...
            if ( $this->_version == self::XLS_BIFF8 ) {
                ...
                if ( $bordersLeftStyle =
                    self::_mapBorderStyle( ( 0x0000000F & self::_GetInt4d( $recordData, 10 ) ) >> 0 )
                ) {
                    $objStyle->getBorders()
                             ->getLeft()
                ...
                if ( $bordersRightStyle =
                    self::_mapBorderStyle( ( 0x000000F0 & self::_GetInt4d( $recordData, 10 ) ) >> 4 )
                ) {
                    $objStyle->getBorders()
                             ->getRight()
                ...
                if ( $bordersTopStyle =
                    self::_mapBorderStyle( ( 0x00000F00 & self::_GetInt4d( $recordData, 10 ) ) >> 8 )
                ) {
                    $objStyle->getBorders()
                             ->getTop()
                ...
                if ( $bordersBottomStyle =
                    self::_mapBorderStyle( ( 0x0000F000 & self::_GetInt4d( $recordData, 10 ) ) >> 12 )
                ) {
                    $objStyle->getBorders()
                             ->getBottom()
                ...
                $objStyle->getBorders()
                         ->getLeft()->colorIndex = ( 0x007F0000 & self::_GetInt4d( $recordData, 10 ) ) >> 16;
                ...
                $objStyle->getBorders()
                         ->getTop()->colorIndex = ( 0x0000007F & self::_GetInt4d( $recordData, 14 ) ) >> 0;
                
                // bit: 13-7; mask: 0x00003F80; bottom color
                $objStyle->getBorders()
                         ->getBottom()->colorIndex = ( 0x00003F80 & self::_GetInt4d( $recordData, 14 ) ) >> 7;
                
                // bit: 20-14; mask: 0x001FC000; diagonal color
                $objStyle->getBorders()
                         ->getDiagonal()->colorIndex = ( 0x001FC000 & self::_GetInt4d( $recordData, 14 ) ) >> 14;
                
                // bit: 24-21; mask: 0x01E00000; diagonal style
                if ( $bordersDiagonalStyle =
                    self::_mapBorderStyle( ( 0x01E00000 & self::_GetInt4d( $recordData, 14 ) ) >> 21 )
                ) {
                    $objStyle->getBorders()
                             ->getDiagonal()
                ...
                $objStyle->getFill()
                         ->setFillType( self::_mapFillPattern( ( 0x003F0000 & $borderAndBackground ) >> 16 ) );
                ...
                switch ( $extType ) {
                    case 4:  // fill start color
                        ...
                        if ( $xclfType == 2 ) {
                            ...
                            if ( isset( $this->_mapCellXfIndex[ $ixfe ] ) ) {
                                $fill = $this->_phpExcel->getCellXfByIndex( $this->_mapCellXfIndex[ $ixfe ] )
                                                        ->getFill();
                                $fill->getStartColor()
                                     ->setRGB( $rgb );
                    ...
                    case 13: // font color
                        ...
                        if ( $xclfType == 2 ) {
                            ...
                            // modify the relevant style property
                            if ( isset( $this->_mapCellXfIndex[ $ixfe ] ) ) {
                                $font = $this->_phpExcel->getCellXfByIndex( $this->_mapCellXfIndex[ $ixfe ] )
                                                        ->getFont();
                                $font->getColor()
                                     ->setRGB( $rgb );
    ...
    private function _readSheet()
    {
        ...
        $this->_sheets[] =
            ['name' => $rec_name, 'offset' => $rec_offset, 'sheetState' => $sheetState, 'sheetType' => $sheetType,];
    }
    ...
    private function _readExternalBook()
    {
        ...
        if ( strlen( $recordData ) > 4 ) {
            ...
            $this->_externalBooks[] = ['type'               => 'external', 'encodedUrl' => $encodedUrlString[ 'value' ],
        ...
        if ( isset( $this->_ref[ $index ] ) ) {
            ...
            switch ( $type ) {
                case 'internal':
                    ...
                    if ( $this->_ref[ $index ][ 'firstSheetIndex' ] == 0xFFFF or
                         $this->_ref[ $index ][ 'lastSheetIndex' ] == 0xFFFF
                    ) {
    ...
    private function _readExternSheet()
    {
        ...
        if ( $this->_version == self::XLS_BIFF8 ) {
            ...
            $nm = self::_GetInt2d( $recordData, 0 );
            for ( $i = 0; $i < $nm; ++$i ) {
                $this->_ref[] = [// offset: 2 + 6 * $i; index to EXTERNALBOOK record
    ...
    private function _readDefinedName()
    {
        ...
        if ( $this->_version == self::XLS_BIFF8 ) {
            ...
            $this->_definedname[] =
                ['isBuiltInName' => $isBuiltInName, 'name' => $string[ 'value' ], 'formula' => $formula,
                 'scope'         => $scope,];
        }
    }
    ...
    private static function _mapBuiltInColor( $color )
    {
        switch ( $color ) {
            ...
            case 0x07:
                return ['rgb' => '00FFFF'];
            case 0x40:
                return ['rgb' => '000000']; // system window text color
            case 0x41:
                return ['rgb' => 'FFFFFF']; // system window background color
            default:
                return ['rgb' => '000000'];
        }
    }
    ...
    private function _readHeader()
    {
        ...
        if ( !$this->_readDataOnly ) {
            ...
            if ( $recordData ) {
                ...
                $this->_phpSheet->getHeaderFooter()
    ...
    private function _readLeftMargin()
    {
        ...
        if ( !$this->_readDataOnly ) {
            // offset: 0; size: 8
            $this->_phpSheet->getPageMargins()
                            ->setLeft( self::_extractNumber( $recordData ) );
        }
    }
    ...
    private function _readRightMargin()
    {
        ...
        if ( !$this->_readDataOnly ) {
            // offset: 0; size: 8
            $this->_phpSheet->getPageMargins()
                            ->setRight( self::_extractNumber( $recordData ) );
        }
    }
    ...
    private function _readTopMargin()
    {
        ...
        if ( !$this->_readDataOnly ) {
            // offset: 0; size: 8
            $this->_phpSheet->getPageMargins()
                            ->setTop( self::_extractNumber( $recordData ) );
        }
    }
    ...
    private function _readBottomMargin()
    {
        ...
        if ( !$this->_readDataOnly ) {
            // offset: 0; size: 8
            $this->_phpSheet->getPageMargins()
                            ->setBottom( self::_extractNumber( $recordData ) );
        }
    }
    ...
    private function _readDefColWidth()
    {
        ...
        if ( $width != 8 ) {
            $this->_phpSheet->getDefaultColumnDimension()
                            ->setWidth( $width );
        }
    }
    ...
    private function _readColInfo()
    {
        ...
        if ( !$this->_readDataOnly ) {
            ...
            for ( $i = $fc; $i <= $lc; ++$i ) {
                ...
                $this->_phpSheet->getColumnDimensionByColumn( $i )
                                ->setWidth( $width / 256 );
                $this->_phpSheet->getColumnDimensionByColumn( $i )
                                ->setVisible( !$isHidden );
                $this->_phpSheet->getColumnDimensionByColumn( $i )
    ...
    private function _readRow()
    {
        ...
        if ( !$this->_readDataOnly ) {
            ...
            if ( !$useDefaultHeight ) {
                $this->_phpSheet->getRowDimension( $r + 1 )
                                ->setRowHeight( $height / 20 );
            }
    ...
    private function _readFormula()
    {
        ...
        if ( ( $this->getReadFilter() !== NULL ) && $this->getReadFilter()
            ...
            if ( ( ord( $recordData{6} ) == 0 ) && ( ord( $recordData{12} ) == 255 ) &&
            ...
            } elseif ( ( ord( $recordData{6} ) == 1 ) && ( ord( $recordData{12} ) == 255 ) &&
                       ( ord( $recordData{13} ) == 255 )
            ) {
            ...
            } elseif ( ( ord( $recordData{6} ) == 2 ) && ( ord( $recordData{12} ) == 255 ) &&
                       ( ord( $recordData{13} ) == 255 )
            ) {
            ...
            } elseif ( ( ord( $recordData{6} ) == 3 ) && ( ord( $recordData{12} ) == 255 ) &&
                       ( ord( $recordData{13} ) == 255 )
            ) {
    ...
    private function _readByteStringLong( $subData )
    {
        ...
        //return $string;
        return ['value' => $value, 'size' => 2 + $ln,// size in bytes of data structure
        ];
    }
        ...
        if ( !$this->_readDataOnly ) {
            for ( $i = 0; $i < $length / 2 - 3; ++$i ) {
                ...
                if ( ( $this->getReadFilter() !== NULL ) && $this->getReadFilter()
                    ...
                    $this->_phpSheet->getCell( $columnString . ( $row + 1 ) )
                                    ->setXfIndex( $this->_mapCellXfIndex[ $xfIndex ] );
                }
            }
        }
    ...
    private function _readObj()
    {
        ...
        $this->_objs[] =
            ['ftCmoType' => $ftCmoType, 'cbCmoSize' => $cbCmoSize, 'otObjType' => $otObjType, 'idObjID' => $idObjID,
             'grbitOpts' => $grbitOpts];
    ...
    private function _readWindow2()
    {
        ...
        if ( $this->_phpSheet->getSheetView()
    ...
    private function _readPageLayoutView()
    {
        ...
        if ( $fPageLayoutView === 1 ) {
            ...
            $this->_phpSheet->getSheetView()
                            ->setZoomScale( $wScalePLV ); //set by Excel2007 only if SHEETVIEW_PAGE_LAYOUT
        }
    ...
    private function _readSelection()
    {
        ...
        if ( !$this->_readDataOnly ) {
            ...
            $cellRangeAddressList =
                $this->_readBIFF5CellRangeAddressList( $data ); // note: also BIFF8 uses BIFF5 syntax
            ...
            for ( $row = $rangeBoundaries[ 0 ][ 1 ]; $row <= $rangeBoundaries[ 1 ][ 1 ]; $row++ ) {
                for ( $column = $rangeBoundaries[ 0 ][ 0 ]; $column != $rangeBoundaries[ 1 ][ 0 ]; $column++ ) {
                    if ( $this->getReadFilter()
                              ->readCell( $column, $row, $this->_phpSheet->getTitle() )
                    ) {
                        $includeCellRange = TRUE;
                        break 2;
                    }
    ...
    private function _readDataValidation()
    {
        ...
        foreach ( $cellRangeAddresses as $cellRange ) {
            ...
            foreach ( $stRange as $coordinate ) {
                $objValidation = $this->_phpSheet->getCell( $coordinate )
                                                 ->getDataValidation();
    ...
    private function _readNote()
    {
        ...
        if ( $this->_version == self::XLS_BIFF8 ) {
            ...
            //
            $this->_cellNotes[ $noteObjID ] =
                ['cellRef' => $cellAddress, 'objectID' => $noteObjID, 'author' => $noteAuthor];
        } else {
            $extension = FALSE;
    ...
    private function _readTextObject()
    {
        ...
        $this->_textObjects[ $this->textObjRef ] =
            ['text'      => substr( $text[ "recordData" ], $text[ "spliceOffsets" ][ 0 ] + 1, $cchText ),
             ...
             'alignment' => $grbitOpts, 'rotation' => $rot];
