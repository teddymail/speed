        ...
        foreach ( $gnmXML->Sheets->Sheet as $sheet ) {
            ...
            foreach ( $sheet->Cells->Cell as $cell ) {
                ...
                if ( $ExprID > '' ) {
                    if ( ( ( string )$cell ) > '' ) {
                        ...
                        $this->_expressions[ $ExprID ] =
                            ['column'  => $cellAttributes->Col, 'row' => $cellAttributes->Row,
                             'formula' => ( string )$cell];
                        //						echo 'NEW EXPRESSION ',$ExprID,'<br />';
                    } else {
                        $expression = $this->_expressions[ $ExprID ];
                        
                        $cell = $this->_referenceHelper->updateFormulaReferences( $expression[ 'formula' ], 'A1',
                                                                                  $cellAttributes->Col -
                                                                                  $expression[ 'column' ],
                                                                                  $cellAttributes->Row -
                                                                                  $expression[ 'row' ],
            ...
            foreach ( $sheet->Styles->StyleRegion as $styleRegion ) {
                ...
                if ( ( $styleAttributes[ 'startRow' ] <= $maxRow ) && ( $styleAttributes[ 'startCol' ] <= $maxCol ) ) {
                    ...
                    $endColumn =
                        ( $styleAttributes[ 'endCol' ] > $maxCol ) ? $maxCol : ( int )$styleAttributes[ 'endCol' ];
                    ...
                    if ( ( !$this->_readDataOnly ) ||
                        ...
                        if ( !$this->_readDataOnly ) {
                            ...
                            $styleArray[ 'alignment' ][ 'wrap' ] =
                                ( $styleAttributes[ 'WrapText' ] == '1' ) ? TRUE : FALSE;
                            ...
                            $styleArray[ 'font' ][ 'strike' ] =
                                ( $fontAttributes[ 'StrikeThrough' ] == '1' ) ? TRUE : FALSE;
            ...
            if ( ( !$this->_readDataOnly ) && ( isset( $sheet->Cols ) ) ) {
                ...
                foreach ( $sheet->Cols->ColInfo as $columnOverride ) {
                    ...
                    $hidden =
                        ( ( isset( $columnAttributes[ 'Hidden' ] ) ) && ( $columnAttributes[ 'Hidden' ] == '1' ) ) ?
                            TRUE : FALSE;
            ...
            if ( ( !$this->_readDataOnly ) && ( isset( $sheet->Rows ) ) ) {
                ...
                foreach ( $sheet->Rows->RowInfo as $rowOverride ) {
                    ...
                    $hidden =
                        ( ( isset( $rowAttributes[ 'Hidden' ] ) ) && ( $rowAttributes[ 'Hidden' ] == '1' ) ) ? TRUE :
                            FALSE;
    ...
    public function listWorksheetInfo( $pFilename )
    {
        ...
        while ( $xml->read() ) {
            if ( $xml->name == 'gnm:Sheet' && $xml->nodeType == XMLReader::ELEMENT ) {
                $tmpInfo = ['worksheetName' => '', 'lastColumnLetter' => 'A', 'lastColumnIndex' => 0, 'totalRows' => 0,
                            'totalColumns'  => 0,];
