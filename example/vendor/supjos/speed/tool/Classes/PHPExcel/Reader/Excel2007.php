...
class PHPExcel_Reader_Excel2007 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    ...
                    if ( !$this->_readDataOnly && $xmlStyles ) {
                        ...
                        foreach ( $xmlStyles->cellStyleXfs->xf as $xf ) {
                            ...
                            $cellStyle = ( object )["numFmt"      => $numFmt,
                                                    "font"        => $xmlStyles->fonts->font[ intval( $xf[ "fontId" ] ) ],
                                                    "fill"        => $xmlStyles->fills->fill[ intval( $xf[ "fillId" ] ) ],
                                                    "border"      => $xmlStyles->borders->border[ intval( $xf[ "borderId" ] ) ],
                                                    "alignment"   => $xf->alignment, "protection" => $xf->protection,
                    ...
                    if ( $xmlWorkbook->sheets ) {
                        foreach ( $xmlWorkbook->sheets->sheet as $eleSheet ) {
                            ...
                            $aKeys = ["sheet", "objects", "scenarios", "formatCells", "formatColumns", "formatRows",
                                ...
                                if ( $xmlSheet && $xmlSheet->legacyDrawingHF && !$this->_readDataOnly ) {
                                    if ( $zip->locateName( dirname( "$dir/$fileWorksheet" ) . "/_rels/" .
                                        ...
                                        if ( $vmlRelationship != '' ) {
                                            ...
                                            // Fetch VML document
                                            $vmlDrawing = simplexml_load_string( $this->_getFromZipArchive( $zip,
                                                                                                            $vmlRelationship ),
                                                                                 'SimpleXMLElement',
                                                                                 PHPExcel_Settings::getLibXmlLoaderOptions() );
                                                ...
                                                if ( $twoCellAnchor->pic->blipFill ) {
                                                    ...
                                                    $chartDetails[ $docSheet->getTitle() . '!' . $thisChart ] =
                                                        ['fromCoordinate' => $fromCoordinate,
                                                         ...
                                                         'toCoordinate'   => $toCoordinate, 'toOffsetX' => $toOffsetX,
                                                         'toOffsetY'      => $toOffsetY,
                                            ...
                                            case '_xlnm.Print_Titles':
                                                ...
                                                foreach ( $extractedRange as $range ) {
                                                    ...
                                                    if ( preg_match( '/!?([A-Z]+)\:([A-Z]+)$/', $range, $matches ) ) {
                                                        $docSheet->getPageSetup()
                                                                 ->setColumnsToRepeatAtLeft( [$matches[ 1 ],
                                                                                                 $matches[ 2 ]] );
                                                    } // check for repeating rows, e.g. '1:1' or '1:5'
                                                    elseif ( preg_match( '/!?(\d+)\:(\d+)$/', $range, $matches ) ) {
                                                        $docSheet->getPageSetup()
                                                                 ->setRowsToRepeatAtTop( [$matches[ 1 ],
                                                                                             $matches[ 2 ]] );
                                                    }
                                                }
                                                break;
    ...
    private function _castToFormula( $c, $r, &$cellDataType, &$value, &$calculatedValue, &$sharedFormulas,
        $castBaseType )
    {
        ...
        foreach ( $rels->Relationship as $rel ) {
            if ( $rel[ "Type" ] ==
                ...
                if ( $xmlWorkbook->sheets ) {
                    $dir = dirname( $rel[ "Target" ] );
                    foreach ( $xmlWorkbook->sheets->sheet as $eleSheet ) {
                        $tmpInfo = ['worksheetName'   => ( string )$eleSheet[ "name" ], 'lastColumnLetter' => 'A',
                                    'lastColumnIndex' => 0, 'totalRows' => 0, 'totalColumns' => 0,];
