...
class PclZip
{
    ...
    function create( $p_filelist )
    {
        ...
        $v_supported_attributes = [PCLZIP_ATT_FILE_NAME           => 'mandatory',
                                   PCLZIP_ATT_FILE_NEW_SHORT_NAME => 'optional',
                                   PCLZIP_ATT_FILE_NEW_FULL_NAME  => 'optional', PCLZIP_ATT_FILE_MTIME => 'optional',
                                   PCLZIP_ATT_FILE_CONTENT        => 'optional', PCLZIP_ATT_FILE_COMMENT => 'optional'];
            ...
            switch ( $p_options_list[ $i ] ) {
                ...
                case PCLZIP_OPT_TEMP_FILE_THRESHOLD :
                    ...
                    if ( isset( $v_result_list[ PCLZIP_OPT_TEMP_FILE_OFF ] ) ) {
                        PclZip::privErrorLog( PCLZIP_ERR_INVALID_PARAMETER,
                                              "Option '" . PclZipUtilOptionText( $p_options_list[ $i ] ) .
                    ...
                    if ( ( !is_integer( $v_value ) ) || ( $v_value < 0 ) ) {
                        PclZip::privErrorLog( PCLZIP_ERR_INVALID_OPTION_VALUE, "Integer expected for option '" .
                                                                               PclZipUtilOptionText( $p_options_list[ $i ] ) .
                                                                               "'" );
            ...
            for ( $key = reset( $v_requested_options ); $key = key( $v_requested_options ); $key =
                ...
                if ( $v_requested_options[ $key ] == 'mandatory' ) {
                    ...
                    if ( !isset( $v_result_list[ $key ] ) ) {
                        // ----- Error log
                        PclZip::privErrorLog( PCLZIP_ERR_INVALID_PARAMETER,
                                              "Missing mandatory parameter " . PclZipUtilOptionText( $key ) . "(" .
                                              $key . ")" );
            ...
            switch ( $v_key ) {
                case PCLZIP_ATT_FILE_NAME :
                    if ( !is_string( $v_value ) ) {
                        PclZip::privErrorLog( PCLZIP_ERR_INVALID_ATTRIBUTE_VALUE,
                                              "Invalid type " . gettype( $v_value ) .
                                              ". String expected for attribute '" . PclZipUtilOptionText( $v_key ) .
                                              "'" );
                ...
                case PCLZIP_ATT_FILE_MTIME :
                    if ( !is_integer( $v_value ) ) {
                        PclZip::privErrorLog( PCLZIP_ERR_INVALID_ATTRIBUTE_VALUE,
                                              "Invalid type " . gettype( $v_value ) .
                                              ". Integer expected for attribute '" . PclZipUtilOptionText( $v_key ) .
                                              "'" );
    ...
    function privWriteCentralFileHeader( &$p_header )
    {
        ...
        // ----- Packed data
        $v_binary_data =
            pack( "VvvvvvvVVVvvvvvVV", 0x02014b50, $p_header[ 'version' ], $p_header[ 'version_extracted' ],
    ...
    function extract()
    {
        ...
        if ( $v_size > 0 ) {
            ...
            if ( ( is_integer( $v_arg_list[ 0 ] ) ) && ( $v_arg_list[ 0 ] > 77000 ) ) {
                ...
                // ----- Parse the options
                $v_result = $this->privParseOptions( $v_arg_list, $v_size, $v_options, [PCLZIP_OPT_PATH                    => 'optional',
                                                                                        PCLZIP_OPT_REMOVE_PATH             => 'optional',
                                                                                        PCLZIP_OPT_REMOVE_ALL_PATH         => 'optional',
                                                                                        PCLZIP_OPT_ADD_PATH                => 'optional',
        ...
        for ( $i = 0, $v_nb_extracted = 0; $i < $v_central_dir[ 'entries' ]; $i++ ) {
            ...
            if ( ( $v_extract ) && ( ( $v_header[ 'compression' ] != 8 ) && ( $v_header[ 'compression' ] != 0 ) ) ) {
                ...
                if ( ( isset( $p_options[ PCLZIP_OPT_STOP_ON_ERROR ] ) ) &&
                    ...
                    PclZip::privErrorLog( PCLZIP_ERR_UNSUPPORTED_COMPRESSION,
                                          ...
                                          "compressed by an unsupported compression " . "method (" .
            ...
            if ( $v_extract ) {
                ...
                if ( $p_options[ PCLZIP_OPT_EXTRACT_AS_STRING ] ) {
                    ...
                    // ----- Get the only interesting attributes
                    if ( ( $v_result =
                            $this->privConvertHeader2FileInfo( $v_header, $p_file_list[ $v_nb_extracted ] ) ) != 1
                    ) {
                        // ----- Close the zip file
                        $this->privCloseFd();
                        $this->privSwapBackMagicQuotes();
    ...
    function privExtractFileUsingTempFile( &$p_entry, &$p_options )
    {
        ...
        $v_binary_data =
            pack( 'va1a1Va1a1', 0x8b1f, Chr( $p_entry[ 'compression' ] ), Chr( 0x00 ), time(), Chr( 0x00 ), Chr( 3 ) );
    ...
    function extractByIndex( $p_index )
    {
        ...
        // ----- Call the extracting fct
        if ( ( $v_result =
                $this->privExtractByRule( $p_list, $v_path, $v_remove_path, $v_remove_all_path, $v_options ) ) < 1
        ) {
            return ( 0 );
        }
    ...
    function privDeleteByRule( &$p_result_list, &$p_options )
    {
        ...
        if ( $v_nb_extracted > 0 ) {
            ...
            // ----- Create the central dir footer
            if ( ( $v_result =
                    $v_temp_zip->privWriteCentralHeader( sizeof( $v_header_list ), $v_size, $v_offset, $v_comment ) ) !=
                 1
            ) {
                // ----- Reset the file list
                unset( $v_header_list );
