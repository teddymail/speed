...
class PHPExcel_Calculation_Financial
{
     ...
     * @return    float
     */
    public static function ACCRINT( $issue, $firstinterest, $settlement, $rate, $par = 1000, $frequency = 1,
        $basis = 0 )
    {
    ...
    public static function AMORDEGRC( $cost, $purchased, $firstPeriod, $salvage, $period, $rate, $basis = 0 )
    {
        ...
        $rate *= $amortiseCoeff;
        $fNRate =
            round( PHPExcel_Calculation_DateTime::YEARFRAC( $purchased, $firstPeriod, $basis ) * $rate * $cost, 0 );
        $cost -= $fNRate;
        $fRest = $cost - $salvage;
    ...
    public static function AMORLINC( $cost, $purchased, $firstPeriod, $salvage, $period, $rate, $basis = 0 )
    {
        ...
        if ( ( $basis == 1 ) && ( $yearFrac < 1 ) &&
             ( PHPExcel_Calculation_DateTime::_isLeapYear( $purchasedYear ) )
        ) {
            $yearFrac *= 365 / 366;
        }
    ...
    public static function COUPNCD( $settlement, $maturity, $frequency, $basis = 0 )
    {
        ...
        if ( ( $settlement > $maturity ) || ( !self::_validFrequency( $frequency ) ) ||
             ( ( $basis < 0 ) || ( $basis > 4 ) )
        ) {
            return PHPExcel_Calculation_Functions::NaN();
        }
    ...
    public static function PMT( $rate = 0, $nper = 0, $pv = 0, $fv = 0, $type = 0 )
    {
        ...
        if ( !is_null( $rate ) && $rate != 0 ) {
            return ( -$fv - $pv * pow( 1 + $rate, $nper ) ) / ( 1 + $rate * $type ) /
    ...
    public static function DB( $cost, $salvage, $life, $period, $month = 12 )
    {
        ...
        if ( ( is_numeric( $cost ) ) && ( is_numeric( $salvage ) ) && ( is_numeric( $life ) ) &&
             ( is_numeric( $period ) ) && ( is_numeric( $month ) )
        ) {
            $cost = ( float )$cost;
            ...
            if ( $cost == 0 ) {
                return 0.0;
            } elseif ( ( $cost < 0 ) || ( ( $salvage / $cost ) < 0 ) || ( $life <= 0 ) || ( $period < 1 ) ||
                       ( $month < 1 )
            ) {
                return PHPExcel_Calculation_Functions::NaN();
            }
    ...
    public static function DDB( $cost, $salvage, $life, $period, $factor = 2.0 )
    {
        ...
        //	Validate
        if ( ( is_numeric( $cost ) ) && ( is_numeric( $salvage ) ) && ( is_numeric( $life ) ) &&
             ( is_numeric( $period ) ) && ( is_numeric( $factor ) )
        ) {
            $cost = ( float )$cost;
            $salvage = ( float )$salvage;
            $life = ( int )$life;
            $period = ( int )$period;
            $factor = ( float )$factor;
            if ( ( $cost <= 0 ) || ( ( $salvage / $cost ) < 0 ) || ( $life <= 0 ) || ( $period < 1 ) ||
                 ( $factor <= 0.0 ) || ( $period > $life )
            ) {
                return PHPExcel_Calculation_Functions::NaN();
            }
            //	Set Fixed Depreciation Rate
    ...
    public static function DISC( $settlement, $maturity, $price, $redemption, $basis = 0 )
    {
        ...
        if ( ( is_numeric( $price ) ) && ( is_numeric( $redemption ) ) && ( is_numeric( $basis ) ) ) {
            ...
            $daysBetweenSettlementAndMaturity =
                PHPExcel_Calculation_DateTime::YEARFRAC( $settlement, $maturity, $basis );
    ...
    public static function NPER( $rate = 0, $pmt = 0, $pv = 0, $fv = 0, $type = 0 )
    {
        ...
        if ( !is_null( $rate ) && $rate != 0 ) {
            ...
            return log( ( $pmt * ( 1 + $rate * $type ) / $rate - $fv ) /
                        ( $pv + $pmt * ( 1 + $rate * $type ) / $rate ) ) / log( 1 + $rate );
        } else {
            if ( $pmt == 0 ) {
                return PHPExcel_Calculation_Functions::NaN();
            }
    ...
    public static function PV( $rate = 0, $nper = 0, $pmt = 0, $fv = 0, $type = 0 )
    {
        ...
        // Calculate
        if ( !is_null( $rate ) && $rate != 0 ) {
            return ( -$pmt * ( 1 + $rate * $type ) * ( ( pow( 1 + $rate, $nper ) - 1 ) / $rate ) - $fv ) /
                   pow( 1 + $rate, $nper );
        } else {
            return -$fv - $pmt * $nper;
        }
    ...
    public static function TBILLEQ( $settlement, $maturity, $discount )
    {
        ...
        if ( PHPExcel_Calculation_Functions::getCompatibilityMode() ==
            ...
            $daysBetweenSettlementAndMaturity = ( PHPExcel_Calculation_DateTime::_getDateValue( $maturity ) -
