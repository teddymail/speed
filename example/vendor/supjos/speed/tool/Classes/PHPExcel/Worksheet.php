...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    public function calculateColumnWidths( $calculateMergeCells = FALSE )
    {
        ...
        if ( !empty( $autoSizes ) ) {
            ...
            foreach ( $this->getCellCollection( FALSE ) as $cellID ) {
                ...
                if ( isset( $autoSizes[ $this->_cellCollection->getCurrentColumn() ] ) ) {
                    ...
                    if ( !isset( $isMergeCell[ $this->_cellCollection->getCurrentAddress() ] ) ) {
                        ...
                        $autoSizes[ $this->_cellCollection->getCurrentColumn() ] =
                            max( ( float )$autoSizes[ $this->_cellCollection->getCurrentColumn() ],
                                 ( float )PHPExcel_Shared_Font::calculateColumnWidth( $this->getParent()
                                                                                           ->getCellXfByIndex( $cell->getXfIndex() )
                                                                                           ->getFont(), $cellValue,
                                                                                      $this->getParent()
                                                                                           ->getCellXfByIndex( $cell->getXfIndex() )
            ...
            foreach ( $autoSizes as $columnIndex => $width ) {
                if ( $width == -1 ) {
                    $width = $this->getDefaultColumnDimension()
                                  ->getWidth();
                }
    ...
    public function getCell( $pCoordinate = 'A1' )
    {
        ...
        if ( strpos( $pCoordinate, '!' ) !== FALSE ) {
            ...
            return $this->_parent->getSheetByName( $worksheetReference[ 0 ] )
                                 ->getCell( $worksheetReference[ 1 ] );
        }
        
        // Named range?
        if ( ( !preg_match( '/^' . PHPExcel_Calculation::CALCULATION_REGEXP_CELLREF . '$/i', $pCoordinate,
                            $matches ) ) &&
             ( preg_match( '/^' . PHPExcel_Calculation::CALCULATION_REGEXP_NAMEDRANGE . '$/i', $pCoordinate,
                           $matches ) )
        ) {
    ...
    {
        $cell = $this->_cellCollection->addCacheData( $pCoordinate,
                                                      new PHPExcel_Cell( NULL, PHPExcel_Cell_DataType::TYPE_NULL,
                                                                         $this ) );
        $this->_cellCollectionIsSorted = FALSE;
        
        // Coordinates
        $aCoordinates = PHPExcel_Cell::coordinateFromString( $pCoordinate );
        if ( PHPExcel_Cell::columnIndexFromString( $this->_cachedHighestColumn ) <
    ...
    {
        $cell = $this->getCellByColumnAndRow( $pColumn, $pRow )
                     ->setValue( $pValue );
     ...
     * @return PHPExcel_Worksheet|PHPExcel_Cell    Depending on the last parameter being specified
     */
    public function setCellValueExplicit( $pCoordinate = 'A1', $pValue = NULL,
        $pDataType = PHPExcel_Cell_DataType::TYPE_STRING, $returnCell = FALSE )
    {
        // Set value
        $cell = $this->getCell( $pCoordinate )
     ...
     * @return PHPExcel_Worksheet|PHPExcel_Cell    Depending on the last parameter being specified
     */
    public function setCellValueExplicitByColumnAndRow( $pColumn = 0, $pRow = 1, $pValue = NULL,
        $pDataType = PHPExcel_Cell_DataType::TYPE_STRING, $returnCell = FALSE )
    {
        $cell = $this->getCellByColumnAndRow( $pColumn, $pRow )
                     ->setValueExplicit( $pValue, $pDataType );
            ...
            if ( $namedRange !== NULL ) {
                ...
                if ( $this->getHashCode() != $namedRange->getWorksheet()
                    ...
                    if ( !$namedRange->getLocalOnly() ) {
                        ...
                        throw new PHPExcel_Exception( 'Named range ' . $namedRange->getName() .
    ...
    public function getHashCode()
    {
        if ( $this->_dirty ) {
            $this->_hash =
                md5( $this->_title . $this->_autoFilter . ( $this->_protection->isProtectionEnabled() ? 't' : 'f' ) .
                     __CLASS__ );
            $this->_dirty = FALSE;
        }
    ...
    {
        $this->_parent->getDefaultStyle()
                      ->applyFromArray( ['font' => ['name' => $pValue->getFont()
                                                                     ->getName(), 'size' => $pValue->getFont()
                                                                                                   ->getSize(),],] );
    ...
    public function duplicateStyle( PHPExcel_Style $pCellStyle = NULL, $pRange = '' )
    {
        ...
        for ( $col = $rangeStart[ 0 ]; $col <= $rangeEnd[ 0 ]; ++$col ) {
            for ( $row = $rangeStart[ 1 ]; $row <= $rangeEnd[ 1 ]; ++$row ) {
                $this->getCell( PHPExcel_Cell::stringFromColumnIndex( $col - 1 ) . $row )
                     ->setXfIndex( $xfIndex );
            }
        }
    ...
    {
        $cellRange = PHPExcel_Cell::stringFromColumnIndex( $pColumn1 ) . $pRow1 . ':' .
     ...
     * @return PHPExcel_Worksheet
     */
    public function protectCellsByColumnAndRow( $pColumn1 = 0, $pRow1 = 1, $pColumn2 = 0, $pRow2 = 1, $pPassword = '',
     ...
     * @return PHPExcel_Worksheet
     */
    public function unprotectCellsByColumnAndRow( $pColumn1 = 0, $pRow1 = 1, $pColumn2 = 0, $pRow2 = 1, $pPassword = '',
        $pAlreadyHashed = FALSE )
    {
        $cellRange = PHPExcel_Cell::stringFromColumnIndex( $pColumn1 ) . $pRow1 . ':' .
                     PHPExcel_Cell::stringFromColumnIndex( $pColumn2 ) . $pRow2;
    ...
    {
        return $this->setAutoFilter( PHPExcel_Cell::stringFromColumnIndex( $pColumn1 ) . $pRow1 . ':' .
                                     PHPExcel_Cell::stringFromColumnIndex( $pColumn2 ) . $pRow2 );
    }
    ...
    public function removeColumn( $pColumn = 'A', $pNumCols = 1 )
    {
        if ( !is_numeric( $pColumn ) ) {
            $pColumn = PHPExcel_Cell::stringFromColumnIndex( PHPExcel_Cell::columnIndexFromString( $pColumn ) - 1 +
                                                             $pNumCols );
            $objReferenceHelper = PHPExcel_ReferenceHelper::getInstance();
     ...
     * @throws PHPExcel_Exception
     */
    public function namedRangeToArray( $pNamedRange = '', $nullValue = NULL, $calculateFormulas = TRUE,
        $formatData = TRUE, $returnCellRef = FALSE )
    {
        ...
        if ( $namedRange !== NULL ) {
            ...
            return $pWorkSheet->rangeToArray( $pCellRange, $nullValue, $calculateFormulas, $formatData,
                                              $returnCellRef );
        }
     ...
     * @return array
     */
    public function rangeToArray( $pRange = 'A1', $nullValue = NULL, $calculateFormulas = TRUE, $formatData = TRUE,
        $returnCellRef = FALSE )
    {
        // Returnvalue
        $returnValue = [];
                    ...
                    if ( $cell->getValue() !== NULL ) {
                        ...
                        if ( $formatData ) {
                            ...
                            $returnValue[ $rRef ][ $cRef ] =
                                PHPExcel_Style_NumberFormat::toFormattedString( $returnValue[ $rRef ][ $cRef ],
                                                                                ( $style &&
    ...
    public function garbageCollect()
    {
        ...
        foreach ( $this->_columnDimensions as $dimension ) {
            $highestColumn =
                max( $highestColumn, PHPExcel_Cell::columnIndexFromString( $dimension->getColumnIndex() ) );
        }
    ...
    public function shrinkRangeToFit( $range )
    {
        ...
        foreach ( $rangeBlocks as &$rangeSet ) {
            ...
            $rangeSet = $rangeBoundaries[ 0 ][ 0 ] . $rangeBoundaries[ 0 ][ 1 ] . ':' . $rangeBoundaries[ 1 ][ 0 ] .
                        $rangeBoundaries[ 1 ][ 1 ];
        }
        unset( $rangeSet );
    ...
    public function __clone()
    {
        foreach ( $this as $key => $val ) {
            ...
            if ( is_object( $val ) || ( is_array( $val ) ) ) {
                ...
                } elseif ( ( $key == '_autoFilter' ) &&
                           ( $this->_autoFilter instanceof PHPExcel_Worksheet_AutoFilter )
                ) {
