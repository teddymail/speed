...
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
     ...
     * @var array
     */
    private static $_dateFormatReplacements = [// first remove escapes related to non-format characters
        '\\'    => '', //	12-hour suffix
     ...
     * @var array
     */
    private static $_dateFormatReplacements24 = ['hh' => 'H', 'h' => 'G'];
    /**
     * Search/replace values to convert Excel date/time format masks hours to PHP format masks (12 hr clock)
     *
     * @var array
     */
    private static $_dateFormatReplacements12 = ['hh' => 'h', 'h' => 'g'];
    /**
     * Format Code
     *
     * @var string
    ...
    private static function _formatAsFraction( &$value, &$format )
    {
        ...
        if ( ( strpos( $format, '0' ) !== FALSE ) || ( strpos( $format, '#' ) !== FALSE ) ||
             ( substr( $format, 0, 3 ) == '? ?' )
        ) {
            if ( $integerPart == 0 ) {
                $integerPart = '';
